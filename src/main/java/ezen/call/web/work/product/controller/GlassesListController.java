...
/**
 * 안경 리스트 요청 처리 세부 컨트롤러
 * @author 강소영
 * /category/category_ng
 */
public class GlassesListController implements HttpController {
	...
	public String process(Map<String, String> paramMap, Map<String, Object> model, HttpServletRequest request,
		...
		String viewName = "/category/category_ng";

		// 이름순정렬 (GET)
		if (request.getQueryString().equalsIgnoreCase("sortName")) {
			List<Product> productList = productService.readProductListCategory("glasses", "product_name");
		...
		// 낮은가격순정렬 (GET)
			...
			List<Product> productList = productService.readProductListPriceSort("glasses", "ASC");
			model.put("productList", productList);
			
	    // 기본정렬-등록순서대로 보여주기 (GET)
		} else if (request.getQueryString().equalsIgnoreCase("ng")) {
			List<Product> productList = productService.readProductListCategory("glasses", "product_number");
			model.put("productList", productList);
		}
		return viewName;
	}
